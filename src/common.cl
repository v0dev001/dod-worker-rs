...
static void memcpy_from_local(uchar *dest, __local uchar *src, size_t n){
  for(int i=0;i<n;i++){
    dest[i] = src[i];
  }
}

static void memcpy_from_global(uchar *dest, __global uchar *src, size_t n){
  for(int i=0;i<n;i++){
    dest[i] = src[i];
  }
}

static void memcpy_to_global(__global uchar *dest, uchar *src, size_t n){
  for(int i=0;i<n;i++){
    dest[i] = src[i];
  }
}
